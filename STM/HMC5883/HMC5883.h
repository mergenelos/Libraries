...
*/
	...
	#define HMC5883_CONF_REG_B_CRBs		0x1F	//These bits must be cleared for correct operation.

#define HMC5883_MODE_REG		0x02			//Read/Write
#define HMC5883_MODE_REG_MASK	0x83			//Valid bits of the MODE register

	#define HMC5883_MODE_REG_HS0		0x80	//Set this pin to enable High Speed I2C, 3400kHz
...
/*
 * The data output [X,Y,Z] registers are two 8-bit registers,
 * data output register A and data output register B.
 * These registers store the measurement result from channel [X,Y,Z].
 * Data output [X,Y,Z] register A contains the MSB from the measurement result,
 * and data output [X,Y,Z] register B contains the LSB from the measurement result.
 * The value stored in these two registers is a 16-bit value in 2’s complement
 * form, whose range is 0xF800 to 0x07FF.
 *
 * When one or more of the output registers are read,
 * new data cannot be placed in any of the output data registers until all
 * six data output registers are read. This requirement also impacts DRDY and RDY,
